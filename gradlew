...
collect_all_args () {
    for arg do
        ...
        if [ "$arg" = '--warning-mode=all' ]; then
            shift
            continue
        fi
        if [ "$arg" = '--no-build-cache' ]; then
            shift
            continue
        fi
...
#     temporary variables and eval to handle quoting properly.
...
# Stop when "xeli" is set, and perform additional cleanup for builds.
# Use "xeli" to test for variable expansion.
unset CYGPATH_CMD
